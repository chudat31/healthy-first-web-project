...
public class ResController {
    ...
    //Xóa 1 cơ sở theo id của cơ sở đó
    @DeleteMapping("/{id}")
    ...
    //Chỉnh sửa thông tin của 1 cơ sở
    @PatchMapping("/{id}")
    public ResponseEntity<ApiResponse> updateRestaurant(@PathVariable("id") Long _id, @RequestBody RestaurantForm _form) {
        Restaurant restaurant = resService.updateRestaurant(_id, _form.getName(), _form.getDistrict(), _form.getPhone());
        RestaurantDto dto = restaurant.toDto();
